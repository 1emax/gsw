...
*/
	...
	$short_request_uri = $request_uri;
	if ($short_request_uri) {
		$slash_position = strrpos ($short_request_uri, "/");
		$short_request_uri = ($slash_position === false) ? $short_request_uri : substr($short_request_uri, $slash_position + 1);
	}
	$parsed_url = parse_url($request_uri);
	$request_uri_path = isset($parsed_url["path"]) ? $parsed_url["path"] : "/";
	...
	} elseif (!preg_match("/^http\:\/\//", $logo_image)) {
	...
	$sql .= " WHERE hl.layout_id=" . $db->tosql($layout_id, INTEGER);
	$sql .= " AND (submenu_page=" . $db->tosql($request_page, TEXT);
	if ($current_page != $request_page) {
		$sql .= " OR submenu_page=" . $db->tosql($current_page, TEXT);
	}
	if ($request_uri_path != $request_page) {
		$sql .= " OR submenu_page=" . $db->tosql($request_uri_path, TEXT);
	}
	$sql .= ")";
	$sql .= " AND match_type>0 ";
	$db->query($sql);
	...
	$sql .= " WHERE " . $header_where . " AND layout_id=" . $db->tosql($settings["layout_id"], INTEGER);
	$sql .= " ORDER BY menu_order ";
	$db->query($sql);
	while ($db->next_record())
	{
		$menu_id = $db->f("menu_id");
		$menu_url = get_custom_friendly_url($db->f("menu_url"));
		...
		$menu_basename = basename($menu_url);
		if ($parent_menu_id == 0) {
			if ($short_request_uri == $menu_basename || $current_page == $menu_basename) {
		...
		if (strlen($menu_title) || $menu_image || $menu_image_active) {
			$menu_values = array(
				"menu_id" => $menu_id, "menu_url" => $menu_url, "menu_title" => $menu_title, "menu_target" => $db->f("menu_target"),
	...
	foreach ($menu as $parent_menu_id => $menus)
	{
		for ($m = 0; $m < sizeof($menus); $m++) {
			...
			if (!preg_match("/^http\:\/\//", $menu_url) && !preg_match("/^https\:\/\//", $menu_url)) {
				$menu_url = $menu_url;
			}
		...
		if (sizeof($submenu) > 0) {
			...
			foreach ($submenu as $parent_submenu_id => $submenus)
			{
				for ($m = 0; $m < sizeof($submenus); $m++) {
					...
					if (!preg_match("/^http\:\/\//", $submenu_url) && !preg_match("/^https\:\/\//", $submenu_url)) {
