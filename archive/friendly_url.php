...
/*
  ...
  ***      Built: Sat Jul 25 04:26:02 2009                                 ***
  ***      http://www.viart.com                                            ***
  ***                                                                      ***
  ****************************************************************************
*/
	...
	//Редиректы
	$need_redirect = 0;
	if (preg_match("/[A-Z]/",$request_uri)) { //С больших букв на маленькие
		$request_uri=strtolower($request_uri);
		$need_redirect = 1;
	}
	if (!preg_match("/(\&)|(\?)/",$request_uri)) {   // Правила при отсутствии $_GET
		if (!preg_match("/(\.php)/",$request_uri)){	//Дописываем php
			$request_uri = ( (strrpos($request_uri, '/') + 1) == strlen($request_uri)) ? substr($request_uri, 0, -1) : $request_uri;
			$request_uri .= '.php';
			$need_redirect = 1;
		} elseif (preg_match("/(\.php\/)/",$request_uri)) {  // Убираем слеш на конце для php
			$request_uri = preg_replace("/(.php\/)/",".php",$request_uri);
			$need_redirect = 1;
		}
	} elseif (!preg_match("/(\.php\?)/",$request_uri)){  // Правила для урлов с $_GET
			$request_uri = str_replace("/?","?",$request_uri);  //Убираем слеш
			$request_uri = str_replace("?",".php?",$request_uri);    // Дописываем .php
			$need_redirect = 1;		
	}
	if ($need_redirect == 1){
