...
*/
	...
	if ($release_id) {
				...
				if (!$user_id) {
					...
					if ($db->next_record()) {
						...
						if ($download_activation == 0) {
							...
							if ($downloads_number >= $max_downloads) {
								$errors = DOWNLOAD_LIMITED;
							}
						}
					} else {
		...
		$sql  = " SELECT id.order_item_id,id.activated,id.max_downloads,id.download_added, ";
		$sql .= " id.download_expiry,i.download_path AS product_path, id.download_path, ";
		$sql .= " i.download_show_terms, i.download_terms_text ";
		$sql .= " FROM (";
		if (isset($site_id)) {
			$sql .= "(";
