...
*/
	...
	if ($category_id) {
		...
		$sql  = " SELECT fc.category_name, fc.short_description, fc.full_description ";
		if (isset($site_id)) {
			$sql .= " FROM (" . $table_prefix . "forum_categories fc";
			$sql .= " LEFT JOIN " . $table_prefix . "forum_categories_sites fcs ON fcs.category_id=fc.category_id)";
			$sql .= " WHERE (fc.sites_all=1 OR fcs.site_id=". $db->tosql($site_id, INTEGER, true, false) . ") ";
		} else {
			$sql .= " FROM " . $table_prefix . "forum_categories fc";
			$sql .= " WHERE fc.sites_all=1 ";					
		}
		$sql .= " AND fc.category_id=" . $db->tosql($category_id, INTEGER);
