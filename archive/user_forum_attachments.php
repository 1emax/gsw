...
*/
	...
	$allowed_attachments = 0; $attachments_types_all = 0; $allowed_user_type_id = 0;
	if ($thread_id) {
		$sql  = " SELECT fc.category_id, fc.category_name, fl.forum_id, fl.forum_name, f.topic, ";
		$sql .= " fl.allowed_attachments, fl.attachments_types_all, fat.user_type_id ";
		$sql .= " FROM (((" . $table_prefix . "forum f ";
		$sql .= " INNER JOIN " . $table_prefix . "forum_list fl ON f.forum_id=fl.forum_id) ";
		$sql .= " INNER JOIN " . $table_prefix . "forum_categories fc ON fl.category_id=fc.category_id) ";
		$sql .= " LEFT JOIN " . $table_prefix . "forum_attachments_types fat ";
		$sql .= " ON (fat.forum_id=fl.forum_id AND fat.user_type_id=". $db->tosql($user_type_id , INTEGER)."))";
		...
		if ($db->next_record()) {
			...
			$allowed_user_type_id = $db->f("user_type_id");
		} else {
			echo NO_TOPICS_MSG;
			exit;
		}
	} elseif ($forum_id) {
		$sql  = " SELECT fc.category_id, fc.category_name, fl.forum_name, ";
		$sql .= " fl.allowed_attachments, fl.attachments_types_all, fat.user_type_id ";
	...
	if (!($allowed_attachments && ($attachments_types_all || $allowed_user_type_id))) {
	...
	if ($operation == "upload")
	{
		...
		if (!strlen($errors))
		{
			...
			if(!@move_uploaded_file($tmp_name, $filepath . $new_filename)) {
				...
				$sql .= " (user_id, forum_id, thread_id, message_id, attachment_status, file_name, file_path, date_added) VALUES (";
