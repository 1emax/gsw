...
*/
	...
	if ($operation == "1") {
		...
		} elseif (!(preg_match("/((.gif)|(.jpg)|(.jpeg)|(.bmp)|(.tiff)|(.tif)|(.png)|(.ico)|(.doc)|(.txt)|(.rtf)|(.pdf))$/i", $filename)) ) {
			$errors = UPLOAD_FORMAT_ERROR;
		}

		$max_image_width = 0; $max_image_height = 0; $max_image_size = 0;
		$ads_images = array();
		...
		if (!strlen($errors) && $filetype == "personal_image") {
			$user_profile = array();
			$sql  = " SELECT setting_name,setting_value FROM " . $table_prefix . "global_settings ";
