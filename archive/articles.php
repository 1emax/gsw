...
*/
	...
	$sql  = " SELECT ac.category_name,ac.friendly_url,ac.short_description, ac.full_description, ac.category_path, ac.parent_category_id, ";
	$sql .= " ac.articles_list_template, ac.articles_order_column,ac.articles_order_direction, ac.article_list_fields, ";
	$sql .= " ac.image_small, ac.image_small_alt, ac.image_large, ac.image_large_alt, ";
	$sql .= " ac.meta_title, ac.meta_keywords, ac.meta_description, ac.total_views, ";
	$sql .= " ac.is_rss, ac.rss_on_breadcrumb, ac.is_remote_rss, ac.remote_rss_url, ac.remote_rss_date_updated, ac.remote_rss_ttl, ac.remote_rss_refresh_rate, ac.user_types_all ";
	$sql .= " FROM ";
	if (isset($site_id)) {
		$sql .= "(";
	}			
	...
	if ($db->next_record()) 
	{
		...
		// check if user could view this category
		if ($user_types_all != 1) {
			if (strlen($user_id)) {
				$sql  = " SELECT * FROM " . $table_prefix . "articles_categories_types ";
				$sql .= " WHERE category_id=" . $db->tosql($category_id, INTEGER);
				$sql .= " AND user_type_id=". $db->tosql($user_type_id, INTEGER, true, false);
				$db->query($sql);
				if (!$db->next_record()) {
