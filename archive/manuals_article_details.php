...
*/
	...
	// retrieve info about current article
	if ($article_id) {
		$sql  = " SELECT ma.article_title";
		$sql .= " FROM ((";
		if (isset($site_id)) {
			$sql .= "(";
		}
		$sql .= $table_prefix . "manuals_articles ma ";
		...
		$sql .= " AND ma.allowed_view = 1 AND ml.allowed_view = 1 AND mc.allowed_view=1 ";	
		if (isset($site_id)) {
			$sql .= " AND (mc.sites_all=1 OR mcs.site_id=". $db->tosql($site_id, INTEGER, true, false) . ") ";
		} else {
			$sql .= " AND mc.sites_all=1 ";					
		}
		$db->query($sql);
		if (!$db->next_record()) {
			header ("Location: " . get_custom_friendly_url("manuals.php"));
			exit;			
		}
	}
