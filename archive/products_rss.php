...
*/
	...
	if ($show_sub_products == 1){
		$sql .= " INNER JOIN " . $table_prefix . "items_categories ic ON i.item_id=ic.item_id AND ic.category_id IN (".$category_ids.")) ";
		$sql .= " LEFT JOIN " . $table_prefix . "categories c ON c.category_id = ic.category_id )";
	} else {
		$sql .= " INNER JOIN " . $table_prefix . "items_categories ic ON i.item_id=ic.item_id AND ic.category_id = ".$category_id.") ";
		$sql .= " LEFT JOIN " . $table_prefix . "categories c ON c.category_id = ic.category_id ) ";
	}
	...
	if ($db->next_record()){
		...
		for ($i = 0; $i < $c; $i++){
			...
			if (preg_match("/http:\/\//",$products_rss[$i]["small_image"])){
				...
				$xml .= "src=\"".xml_entities($settings["site_url"]."images/".$products_rss[$i]["small_image"]);
			}
