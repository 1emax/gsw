...
*/
	...
	$sql .= " AND i.is_showing=1 AND i.is_approved=1 AND ((i.hide_out_of_stock=1 AND i.stock_level > 0) OR i.hide_out_of_stock=0 OR i.hide_out_of_stock IS NULL)";
	if (isset($site_id)) {
	...
	if ($db->next_record())
	{
	  ...
	  $dbpv->DBPersistent = $db->DBPersistent;
		...
		$sql .= " INNER JOIN " . $table_prefix . "items_accessories ia ON ";
		$sql .= " (i.item_id=ia.accessory_id AND ia.item_id=" . $db->tosql($item_id, INTEGER) . ")) ";
		...
		if ($total_accessories > 0) {
			...
			$sql .= " i.buying_price, i.properties_price, i." . $price_field . ", i." . $sales_field . ", i.is_sales, i.tax_free ";
			$sql .= " FROM ((";
			if (isset($site_id)) {
				$sql .= "(";
			}
